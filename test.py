...
def test_metric(model, data_dir, split="test", w=224, h=224, dist="cos", skip_factor=4):
    ...
    t = time.time()
    test_dataset = RealContrastiveDatasetWithInters(data_dir / split, w=w, h=h, skip_factor=skip_factor)
    ...
    t = time.time()
    train_dataset = RealContrastiveDatasetWithInters(data_dir / "train", w=w, h=h, skip_factor=skip_factor)
...
if __name__ == "__main__":
    ...
    # background image taken, the next three are skipped. This is only done to background images.
    acc_dict, avg_acc_dict = test_metric(model, data_path, split="test", skip_factor=4)
