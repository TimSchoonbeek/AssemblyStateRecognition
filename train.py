...
if __name__ == "__main__":
    ...
    if args.loss == "ce":
        model = ContrastiveModel(args, weights_dir=Path(args.run_path).parent / "models", classifier=True)
    else:
        model = ContrastiveModel(args, weights_dir=Path(args.run_path).parent / "models")
    start_epoch = 0
    ...
    train_dataset = ContrastiveDataset(args)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        acc_dict, avg_acc_dict = test_metric(model, Path(args.data_path), split='val', w=args.img_w, h=args.img_h,
                                             dist=dist)
